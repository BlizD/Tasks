...
// Форма получена копированием типовой формы "Обработка.РаботаСФайлами.Форма.ПрисоединенныеФайлы",
// и доработана т.о., чтобы оборажались присоединенные файлы электронных писем, связанных с задачей.
// Все необходимые для этого доработки обособлены комментарием:
// ++ УправлениеЗадачами
// ...
// -- УправлениеЗадачами
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// ++ УправлениеЗадачами
	// Реальный владелец файлов всегда электронное письмо исходящее/входящее.
	// Кого из них взять в данном контексте не имеет значения, т.к. принцип хранения файлов один.
	ТипВладельца = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее");
	// -- УправлениеЗадачами
	...
	КонецЕсли;
	// ++ УправлениеЗадачами: всегда используем форму лишь для просмотра вложений
	СкрытьКнопкиДобавления();
	ЕстьПравоДобавления = Ложь;
	// -- УправлениеЗадачами
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	//Павлюков - 196 - замена ВыбраннаяСтрока на ТекущиеДанные.Ссылка
...
Процедура НастроитьДинамическийСписок(ИмяСправочникаХранилищаФайлов, Знач ЕстьРеквизитСлужебный = Ложь)
	...
	//++ УправлениеЗадачами: дополнительная колонка для картинки письма
	|	, &Направление КАК узНаправлениеПисьма
	//-- УправлениеЗадачами
	|ИЗ
	...
	//++ УправлениеЗадачами: итоговый текст запроса объединяет данные из двух источников.
	...
	ПолноеИмяСправочника = Неопределено;
	
	МассивЧастейЗапроса = Новый Массив;
	
	ТекстПодзапроса = ТекстЗапроса;
	ТекстПодзапроса = СтрЗаменить(ТекстПодзапроса, "&ИмяСправочника", "Справочник.ЭлектронноеПисьмоВходящееПрисоединенныеФайлы");
	ТекстПодзапроса = СтрЗаменить(ТекстПодзапроса, "&Служебный", "ЛОЖЬ");
	ТекстПодзапроса = СтрЗаменить(ТекстПодзапроса, "&ЭтоГруппа", "ЛОЖЬ");
	ТекстПодзапроса = СтрЗаменить(ТекстПодзапроса, "Файлы.ВладелецФайла = &ВладелецФайлов", "ПредметыПапкиВзаимодействий.Предмет = &ВладелецФайлов");
	ТекстПодзапроса = СтрЗаменить(ТекстПодзапроса, "&Направление", "3");
	
	МассивЧастейЗапроса.Добавить(ТекстПодзапроса);
	
	ТекстПодзапроса = ТекстЗапроса;
	ТекстПодзапроса = СтрЗаменить(ТекстПодзапроса, "&ИмяСправочника", "Справочник.ЭлектронноеПисьмоИсходящееПрисоединенныеФайлы");
	ТекстПодзапроса = СтрЗаменить(ТекстПодзапроса, "&Служебный", "ЛОЖЬ");
	ТекстПодзапроса = СтрЗаменить(ТекстПодзапроса, "&ЭтоГруппа", "ЛОЖЬ");
	ТекстПодзапроса = СтрЗаменить(ТекстПодзапроса, "Файлы.ВладелецФайла = &ВладелецФайлов", "ПредметыПапкиВзаимодействий.Предмет = &ВладелецФайлов");
	ТекстПодзапроса = СтрЗаменить(ТекстПодзапроса, "&Направление", "4");
	...
	//++ УправлениеЗадачами: добавление колонки направления письма
	КолонкаФормы = Элементы.Вставить("узНаправлениеПисьма", Тип("ПолеФормы"), Элементы.Список, Элементы.СписокНомерКартинкиПодписанЗашифрован);
	КолонкаФормы.ПутьКДанным = "Список.узНаправлениеПисьма";
	КолонкаФормы.Вид = ВидПоляФормы.ПолеКартинки;
	КолонкаФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	КолонкаФормы.КартинкаЗначений = БиблиотекаКартинок.Взаимодействия;
	КолонкаФормы.КартинкаШапки = БиблиотекаКартинок.ЭлектронноеПисьмо;
	//-- УправлениеЗадачами
КонецПроцедуры
