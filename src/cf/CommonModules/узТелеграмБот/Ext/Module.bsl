...
Функция ПолучитьНовыеСообщения(MaxMessageID = "", Ответ = Неопределено, НеПолучатьПовторно = Истина) Экспорт
	...
	Для Каждого Update Из ОтветСоответствие["result"] Цикл
		...
		// [+] #364 Эдуард Суровый 15.10.2025
		// Обход ошибки при отправке сообщения боту из группы. 
		Если (ДанныеСообщения.Свойство("Текст") И НЕ ПустаяСтрока(ДанныеСообщения.Текст)) 
			ИЛИ (ДанныеСообщения.Свойство("Файлы") И ДанныеСообщения.Файлы.Количество() > 0) Тогда
		// [-] #364 Эдуард Суровый 15.10.2025		
...
Процедура ОтправитьСообщение(ТекстСообщения, Получатели, УведомлятьПользователя = Ложь, Ответ = Неопределено) Экспорт
	...
	ЭкранироватьСпецСимволыСообщенияДляMarkdown(ТекстСообщения);	
...
Процедура ЭкранироватьСпецСимволыСообщенияДляMarkdown(ТекстСообщения) 
	
	//  – Для MarkdownV2 экранируйте: _ * [ ] ( ) ~  > # + - = | { } . ! 
	// (перед каждым добавлять обратный слеш `).
	//СтрокаСпецСимволы = "_*[]()~>#+-=|{}.!";
	СтрокаСпецСимволы = "_";
	НомерСимвола = 1;
	СимволТекст = Сред(СтрокаСпецСимволы, НомерСимвола, 1); 
	Пока НомерСимвола <= СтрДлина(СтрокаСпецСимволы) Цикл
		Если СтрНайти(ТекстСообщения, СимволТекст) > 0 Тогда
			ТекстСообщения = СтрЗаменить(ТекстСообщения, СимволТекст, "\" + СимволТекст);
		КонецЕсли;
...
Функция ВыполнитьЗапросHTTP(НастройкиБота, Команда, ПараметрыЗапроса, Ответ, ЭтоЗапросPOST = Истина) Экспорт
	...
	Пока Истина Цикл
		...
		Если НЕ ОтветУспешен Тогда
         	ОтветСтрокаJSON = Ответ.ПолучитьТелоКакСтроку(); 
			ЗаписьЖурналаРегистрации("Телеграмм.Отправка", УровеньЖурналаРегистрации.Ошибка, , , ОтветСтрокаJSON);
		КонецЕсли;		
