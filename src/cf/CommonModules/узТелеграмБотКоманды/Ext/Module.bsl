...
Функция ДоступныеКоманды()
	...
	//[+] #365 Эдуард Суровый 15.10.2025
	ДоступныеКоманды.Добавить("#НомерЗадачи Статус ТекстСтатуса");
	ДоступныеКоманды.Добавить("#НомерЗадачи @ИмяИсполнителя");
	ДоступныеКоманды.Добавить("/МоиЗадачи - проверить все задачи в работе кроме статуса Готово, /МоиЗадачи Исполнитель - задачи по конкретному исполнителю");
	ДоступныеКоманды.Добавить("/МоиЗадачи Исполнитель - задачи по конкретному исполнителю");
	//[-] #365 Эдуард Суровый 15.10.2025
...
Функция ПроверитьМоиЗадачи(Сообщение)
	...
	ИмяИсполнителя = СокрЛП(Сред(СокрЛ(Сообщение.Текст),11));
	...
	|ГДЕ
	|	узЗадачи.Статус.Наименование <> ""Готово""
	|	И НЕ узЗадачи.ПометкаУдаления
	|	И узЗадачи.Исполнитель.Наименование = &Исполнитель
	|
	|УПОРЯДОЧИТЬ ПО
	|	Код";
	Если ЗначениеЗаполнено(ИмяИсполнителя) Тогда
		Запрос.УстановитьПараметр("Исполнитель",ИмяИсполнителя);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"И узЗадачи.Исполнитель.Наименование = &Исполнитель","");
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = ТекстСообщения + ?(ТекстСообщения="","",Символы.ПС+Символы.ПС) +	ОписаниеЗадачи;
...
Функция ПроверитьНовуюЗадачу(Сообщение, ЗадачаСсылка = Неопределено)
	...
	//[+] #365 Эдуард Суровый 15.10.2025 
	//Позволяет создать задачу и сразу указать Исполнителя (справочник Пользователи). Можно указать, можно не указывать.
