...
Процедура ОбработатьКомандыСообщения(НастройкиБота, Сообщение) Экспорт
	...
	//[+] #365 Эдуард Суровый 15.10.2025
	Если ПроверитьМоиЗадачи(Сообщение) Тогда
		Возврат;
	КонецЕсли;
	//[-] #365 Эдуард Суровый 15.10.2025
	...
	Если НачинаетсяСНомераЗадачи(Сообщение, ЗадачаСсылка) Тогда
		...
		//[+] #365 Эдуард Суровый 15.10.2025
		ИЛИ ПроверитьНовогоИсполнителя(Сообщение, ЗадачаСсылка)
		ИЛИ ПроверитьНовыйСтатус(Сообщение, ЗадачаСсылка)
		//[-] #365 Эдуард Суровый 15.10.2025
...
Функция ДоступныеКоманды()
	...
	ДоступныеКоманды.Добавить("#ТекстОписания @ИмяИсполнителя - новая задача и имя исполнителя (необязательный)");
	...
	//[+] #365 Эдуард Суровый 15.10.2025
	ДоступныеКоманды.Добавить("#НомерЗадачи $НовыйСтатус");
	ДоступныеКоманды.Добавить("#НомерЗадачи @ИмяИсполнителя");
	ДоступныеКоманды.Добавить("/МоиЗадачи - проверить все задачи в работе кроме вида статуса Готово");
	ДоступныеКоманды.Добавить("/МоиЗадачи @ИмяИсполнителя - задачи по конкретному исполнителю");
	//[-] #365 Эдуард Суровый 15.10.2025
...
// [+] #365 Эдуард Суровый 15.10.2025 
// Вывести список задач по пользователю или по всем
Функция ПроверитьМоиЗадачи(Сообщение)
	
	Если Сообщение.Обработано Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если Не СтрНачинаетсяС(СокрП(Сообщение.Текст), "/МоиЗадачи") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ИмяИсполнителя = СокрЛП(Сред(СокрЛ(Сообщение.Текст), 13));
	...
	|	И узЗадачи.Исполнитель = &Исполнитель
	|
	|УПОРЯДОЧИТЬ ПО
	|	Код";
	Если ЗначениеЗаполнено(ИмяИсполнителя) Тогда
	...
	ТекстСообщения = "Всего задач: " + Выборка.Количество();
	
	Пока Выборка.Следующий() Цикл
		
		ОписаниеЗадачи = "#" + Формат(Выборка.Код,"ЧГ=") 
			+ " "                          
			+ СокрЛП(Выборка.Наименование) + Символы.ПС
			+ " Статус: " + СокрЛП(Выборка.Статус) + Символы.ПС
			+ " Исполнитель: " + СокрЛП(Выборка.Исполнитель);
			
		ТекстСообщения = ТекстСообщения 
			+ ?(ТекстСообщения = "", "", Символы.ПС + Символы.ПС) +	ОписаниеЗадачи;
			
	КонецЦикла;
	
	Если ТекстСообщения = "" Тогда
		ТекстСообщения = "Задачи отсутствуют";
	КонецЕсли; 
	
	ВставитьДляЗадачиХештег(ТекстСообщения);
...
Функция ПроверитьНовыйСтатус(Сообщение, ЗадачаСсылка = Неопределено)
	
	Если Сообщение.Обработано Тогда
		Возврат Истина;
	КонецЕсли;
	
	//НовыйСтатус - "Статус ТекстСтатуса"
	//НовыйСтатусКЗадаче - "#НомерЗадачи Статус ТекстСтатуса"
	//[+] #365 Иванов АБ 18.10.2025 
	//Если СтрНачинаетсяС(ВРег(Сообщение.Текст), "СТАТУС") Тогда
	//	Сообщение.Текст = СокрЛ(Сред(Сообщение.Текст, 7));	            
	Если СтрНачинаетсяС(ВРег(Сообщение.Текст), "$") Тогда
		Сообщение.Текст = СокрЛ(Сред(Сообщение.Текст, 2));	
	//[-] #365 Иванов АБ 18.10.2025 	
	Иначе
		Возврат Ложь;
	...
	Статус = Справочники.узСтатусыЗадачи.НайтиПоНаименованию(Сообщение.Текст, Истина);
	...
	пНомерЗадачи = Справочники.узЗадачи.ПолучитьНомерЗадачи(ЗадачаСсылка);
	ТекстСообщения = "#" + пНомерЗадачи+ " " + ЗадачаСсылка + " новый статус: " + Сообщение.Текст;	
...
//[+] #365 Эдуард Суровый 15.10.2025
//Установить исполнителя по задачи
Функция ПроверитьНовогоИсполнителя(Сообщение, ЗадачаСсылка = Неопределено)
	
	Если Сообщение.Обработано Тогда
		Возврат Истина;
	КонецЕсли;
	
	//НовыйИсполнительКЗадаче - "#НомерЗадачи @ИмяИсполнителя"
	...
	Исполнитель = Справочники.Пользователи.НайтиПоНаименованию(Сообщение.Текст, Истина);
	...
	пНомерЗадачи = Справочники.узЗадачи.ПолучитьНомерЗадачи(ЗадачаСсылка);
	ТекстСообщения = "#" + пНомерЗадачи+ " " + ЗадачаСсылка + " новый исполнитель: " + Сообщение.Текст;	
...
Функция ПроверитьНовуюЗадачу(Сообщение, ЗадачаСсылка = Неопределено)
	...
	//[+] #365 Эдуард Суровый 15.10.2025 
	//Позволяет создать задачу и сразу указать Исполнителя (справочник Пользователи). 
	//Можно указать, можно не указывать.
	Исполнитель = Неопределено;
	ПозИсп = СтрНайти(Сообщение.Текст, "@");
	Если ПозИсп > 0 Тогда
		ИмяИсполнителя = СокрЛП(Сред(Сообщение.Текст, ПозИсп + 1));
		Сообщение.Текст = СокрЛП(Лев(Сообщение.Текст, ПозИсп - 1));
		Если ЗначениеЗаполнено(ИмяИсполнителя) Тогда
			Исполнитель = Справочники.Пользователи.НайтиПоНаименованию(ИмяИсполнителя, Истина);
		КонецЕсли;
	КонецЕсли;
	...
	ЗадачаОбъект.ОформлениеТекста = Перечисления.узОформлениеТекста.ФорматированныйТекст;
	ФорматированныйТекст = Новый ФорматированныйДокумент;
	ФорматированныйТекст.УстановитьHTML(ЗадачаОбъект.ТекстСодержания, Новый Структура);
	ЗадачаОбъект.Содержание = Новый ХранилищеЗначения(ФорматированныйТекст, Новый СжатиеДанных(9));
	//[-] #365 Иванов АБ 18.10.2025 
	...
	ТекстСообщения = НСтр("ru = 'Добавлена #%1 %2 %3'");
	ТекстСообщения = СтрШаблон(ТекстСообщения, пНомерЗадачи, ЗадачаОбъект.Наименование, Исполнитель);	
...
Процедура ВставитьДляЗадачиХештег(ТекстСообщения)
	
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "#", "#Задача");	
