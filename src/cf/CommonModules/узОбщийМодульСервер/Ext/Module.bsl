...
// Возвращает навигационную ссылку на HTML-документ markdown-просмотрщика
// (общий макет узMarkdownViewer), помещенный во временное хранилище.
// Ссылку следует назначить реквизиту формы с полем HTML документа.
Функция ПолучитьHTMLMarkdown(ЗНАЧ УникальныйИдентификатор) Экспорт
	ТекстПросмотрщика = ПолучитьОбщийМакет("узMarkdownViewer").ПолучитьТекст();
	
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла("html");
	Запись = Новый ЗаписьТекста(ИмяВременногоФайла, КодировкаТекста.UTF8);
	Запись.Записать(ТекстПросмотрщика);
	Запись.Закрыть();
...
// Экранирует текст для безопасной передачи в JS-строку (в кавычках).
Функция ЭкранироватьСтрокуДляJavaScript(Знач Строка) Экспорт
...
// Экранирует текст для безопасной вставки в HTML-разметку.
Функция ЭкранироватьСтрокуДляHTML(Знач Строка) Экспорт
	Строка = СтрЗаменить(Строка, "&", "&amp;");
	Строка = СтрЗаменить(Строка, "<", "&lt;");
	Строка = СтрЗаменить(Строка, ">", "&gt;");
	Строка = СтрЗаменить(Строка, """", "&quot;");
