...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Markdown-просмотрщик: загружаем автономный HTML-документ навигационной ссылкой
	Если Объект.ОформлениеТекста = ПредопределенноеЗначение("Перечисление.узОформлениеТекста.Markdown") Тогда
		ПолеHTML = узОбщийМодульСервер.ПолучитьHTMLMarkdown(УникальныйИдентификатор);
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступность()	
	...
	УстановитьВидимостьДоступность_ДополнительныеВкладки();
...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.ГруппаСтраницаСкрытьРаскрыть Тогда
		...
		ДополнительныеВкладкиРаскрыты = НЕ ДополнительныеВкладкиРаскрыты;
...
&НаСервере
Процедура УстановитьВидимостьДоступность_ДополнительныеВкладки()   
	
	Элементы.ГруппаСтраницаВопросыИОтветы.Видимость = ДополнительныеВкладкиРаскрыты;
	Элементы.ГруппаСтраницаДополнительно.Видимость = ДополнительныеВкладкиРаскрыты;
	Если Объект.ЭтоОсновнаяЗадача Тогда
		Элементы.ГруппаСтраницаНастройкиОсновнаяЗадача.Видимость = ДополнительныеВкладкиРаскрыты;
	Иначе
		Элементы.ГруппаСтраницаНастройкиОсновнаяЗадача.Видимость = Ложь;
	Конецесли;		
	Элементы.ГруппаСтраницаУчетВремени.Видимость = ДополнительныеВкладкиРаскрыты;
	Элементы.ГруппаСтраницаИстория.Видимость = ДополнительныеВкладкиРаскрыты;
	Элементы.ГруппаСтраницаИзмененныеОбъекты.Видимость = ДополнительныеВкладкиРаскрыты;
...
Процедура ПолучитьMarkdown()
	Если ПолеHTMLПросмотрщикСформирован Тогда
		ОтобразитьMarkdownВПросмотрщике();
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПолеHTMLДокументСформирован(Элемент)
	ПолеHTMLПросмотрщикСформирован = Истина;
	Если Объект.ОформлениеТекста = ПредопределенноеЗначение("Перечисление.узОформлениеТекста.Markdown")
		И Элементы.ГруппаСтраницыОформлениеСодержания.ТекущаяСтраница = Элементы.ГруппаСтраницаПросмотр Тогда
		ОтобразитьMarkdownВПросмотрщике();
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ОтобразитьMarkdownВПросмотрщике()
	Просмотрщик = Элементы.ПолеHTML.Документ["defaultView"];
	
	Поддерживается = Неопределено;
	Попытка
		Поддерживается = Просмотрщик.isMarkdownSupported();
	Исключение
	КонецПопытки;
	
	Если ТипЗнч(Поддерживается) <> Тип("Булево") Тогда
		// Просмотрщик не загрузился — выводим текст без форматирования
		ПоказатьMarkdownБезПросмотрщика();
		Возврат;
	КонецЕсли;
	Если НЕ Поддерживается Тогда
		Сообщить(НСтр("ru = 'Markdown-просмотрщик не поддерживается текущей средой.'"));
		ПоказатьMarkdownБезПросмотрщика();
		Возврат;
	КонецЕсли;
	
	Попытка
		Просмотрщик.renderMarkdown(ТекстДляMarkdownПросмотра());
	Исключение
		ПоказатьMarkdownБезПросмотрщика();
	КонецПопытки;
КонецПроцедуры
...
&НаКлиенте
Процедура ПоказатьMarkdownБезПросмотрщика()
	ПолеHTML = ПолучитьMarkdownБезПросмотрщикаНаСервере(Объект.ТекстСодержания);
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьMarkdownБезПросмотрщикаНаСервере(ЗНАЧ ТекстСодержания)
	Текст = узОбщийМодульСервер.ЭкранироватьСтрокуДляHTML(ТекстСодержания);
	Возврат "<html><body style=""background-color:#ffffff""><pre style=""font-family:monospace"">"
		+ Текст + "</pre></body></html>";
КонецФункции

&НаСервере
Функция ТекстДляMarkdownПросмотра()
	Результат = ПроставитьНавигационныеСсылкиДляЗадачУказанныхВСодержании(Объект.ТекстСодержания);
	Если Результат.ЗаменитьТекстHTML Тогда
		Возврат Результат.НовыйТекстHTML;
	КонецЕсли;
	Возврат Объект.ТекстСодержания;
КонецФункции
...
Процедура ОформлениеТекстаПриИзмененииНаСервере()
	...
	ИначеЕсли Объект.ОформлениеТекста = ПредопределенноеЗначение("Перечисление.узОформлениеТекста.Markdown") Тогда
		...
		Если НЕ ЗначениеЗаполнено(ПолеHTML) Тогда
			ПолеHTML = узОбщийМодульСервер.ПолучитьHTMLMarkdown(УникальныйИдентификатор);
		КонецЕсли;
